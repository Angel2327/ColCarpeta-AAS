...
### AD-11. Arquitectura del portal del ciudadano

**Problema.** El caso de estudio exige una interfaz para que el ciudadano use su carpeta (registrarse, iniciar sesión, cargar y consultar documentos) con la usabilidad como máxima prioridad, incluido el uso desde el teléfono. Definir con qué tecnología se construye esa interfaz y cómo se relaciona con la API ya construida.

**Alternativas.** Aplicación de página única (React, Vue o similar) que consume la API JSON existente, servida como una unidad desplegable aparte; plantillas del lado del servidor renderizadas por la misma aplicación, con HTMX para la interactividad que exige una lista de documentos (subir, eliminar y ver el detalle sin recargar la página completa); plantillas del lado del servidor sin ninguna capa de interactividad, con recarga completa en cada acción.

**Decisión.** Plantillas Jinja2 con HTMX, servidas por el mismo proceso de FastAPI que expone la API. Sin framework de JavaScript, sin paso de compilación, sin segundo despliegue. El portal nunca se llama a sí mismo por HTTP: la lógica de negocio que antes vivía directamente en las rutas JSON (`app.identidad.registro`, `app.identidad.sesion`, `app.documentos.router`) se extrajo a una capa de servicios compartida (`app.identidad.servicios`, `app.documentos.servicios`), y tanto las rutas JSON como las pantallas HTML del portal llaman a esas mismas funciones. La sesión del portal viaja en una cookie HttpOnly, Secure y SameSite=Strict que transporta el mismo JWT que ya emite la API; el JWT como tal no cambia, ni deja de existir para los consumidores de la API.

**Justificación.** Una aplicación de página única aparte contradice AD-01, que fija una sola unidad desplegable: exigiría un segundo proceso de build, un segundo despliegue y una superficie de CORS que no aporta nada a este alcance. Hacer que esa aplicación llamara a la API propia por HTTP sería además que el servidor saliera a la red para volver a entrar a sí mismo, un salto de red y de serialización sin ningún beneficio frente a compartir la función en el mismo proceso. Plantillas sin ninguna interactividad cumplirían el requisito funcional pero no la prioridad de usabilidad: eliminar un documento o ver el resultado de una carga exigiría una recarga completa de la página en cada acción. HTMX cubre esa brecha sin exigir un paso de compilación ni una segunda unidad desplegable, y sin sacar la interactividad del control del servidor: cada acción sigue siendo una petición HTTP corriente que la misma capa de servicios resuelve. La cookie HttpOnly impide que un script de terceros lea el token de sesión, y SameSite protege los formularios del portal contra falsificación de peticiones entre sitios sin necesitar un mecanismo de token CSRF aparte.

**Consecuencias.** Las rutas JSON existentes (`/api/v1/...`) no cambian su contrato: siguen aceptando y devolviendo JSON exactamente igual que antes, solo que ahora delegan en la capa de servicios en vez de contener la lógica ellas mismas. El portal vive en la raíz del dominio (`/`, `/sesion`, `/registro`, `/carpeta`, `/documentos/{id}`, ...) y no aparece en el esquema OpenAPI: no es parte del contrato de la API, es una vista sobre la misma aplicación. La raíz (`/`) deja de responder 404 -- lleva a la carpeta si hay sesión, o a iniciar sesión si no. La cookie de sesión y el `Authorization: Bearer` de la API son dos transportes independientes del mismo JWT: cerrar sesión en el portal borra la cookie pero no invalida el token si se usó también contra la API directamente (mismo comportamiento ya documentado para `DELETE /api/v1/sesion`). Probar el portal en Docker exige que la cookie no lleve el atributo `Secure` cuando la instancia de prueba habla por HTTP simple entre contenedores (`PORTAL_COOKIE_SECURE=false`), exactamente el mismo patrón ya usado para `TRANSFERENCIA_EXIGIR_HTTPS`.

El portal vivió inicialmente bajo `/portal/...` y se movió a la raíz del dominio poco después: la raíz es lo primero que ve cualquiera que entre al dominio, y dejarla en blanco (404) no tiene sentido para una herramienta de autoservicio ciudadano. Cada ruta vieja bajo `/portal/...` redirige de forma permanente (308, que preserva método y cuerpo) hacia su equivalente nueva, por si algún enlace quedó guardado.

### AD-12. Consola de administración (RF32-RF37, CU-22)

**Problema.** El caso de estudio exige una forma de que el operador vea el estado interno del sistema (RF32-RF37; la tabla de casos de uso de esta sección la cataloga como CU-22, "Operar consola de administración" -- CU-19, "Consultar auditoría de accesos", es un caso de uso distinto, del lado del ciudadano, que sigue sin implementar): cuántos ciudadanos hay y en qué estado, cómo va la bandeja de salida, qué transferencias se han hecho. Sin esta consola no existe ninguna forma de observar, por ejemplo, que otro operador nos transfirió un ciudadano: CU-16 no deja una fila propia en ninguna tabla, solo entradas de `outbox` y de `auditoria` (ver "Interoperabilidad entre operadores").

**Alternativas para el alcance de la consola.** Una consola con acciones (reintentar una entrada de la bandeja de salida, revocar una entidad emisora, forzar una purga) además de lectura; una consola de solo lectura; no construir nada y depender de consultas directas a la base de datos.

**Decisión (solo lectura).** La consola es exclusivamente de lectura: ninguna pantalla tiene un botón que borre, reintente, edite ni dispare una operación de negocio.

**Justificación.** Toda la arquitectura del proyecto empuja las operaciones sensibles hacia la bandeja de salida, con sus propios reintentos, su propia espera exponencial y su propia recuperación de filas colgadas (`app.interoperabilidad.outbox`). Una consola con botones de acción reintroduciría, para el operador humano, exactamente el tipo de camino directo y sin control que el resto del sistema evita a propósito para el ciudadano y para los demás operadores -- y lo haría además sin la protección de una revisión de par ni de una prueba automatizada, solo la de quien esté mirando la pantalla en ese momento. Si en el futuro hace falta una acción real (reintentar una entrada fallida, por ejemplo), esa es una decisión aparte, con su propio análisis de qué controles necesita -- no algo que se cuela como efecto secundario de construir la primera versión de la consola. Depender de consultas directas a la base de datos (como ya hacen varios scripts de prueba de este proyecto) no es viable para un uso operativo real: exige acceso a las credenciales de producción para cada consulta, no dice nada por sí sola sobre cómo interpretar el estado, y no deja ningún rastro de quién miró qué.

**Decisión (nunca expone documentos).** La consola nunca muestra el contenido de un documento ni genera un enlace hacia él -- ni de descarga ni una URL firmada. Solo expone metadatos: título, tipo, entidad emisora, estado, tamaño. Tampoco muestra, ni siquiera truncados, ningún secreto: hash de contraseña, secreto TOTP, token de primer acceso, clave de una entidad emisora.

**Justificación.** El caso de estudio pone la confidencialidad de la carpeta ciudadana por delante de cualquier otra prioridad (RNF de confidencialidad; AD-03, "Exposición de documentos a terceros"). Una consola que dejara al operador abrir la carpeta de cualquier ciudadano -- aunque sea con la mejor intención de diagnóstico -- sería una puerta trasera hacia todas las carpetas del sistema, exactamente el tipo de acceso que el resto de la arquitectura (enlaces firmados de vigencia corta, nunca un objeto público del bucket) existe para evitar. Los metadatos alcanzan para todo lo que un administrador necesita diagnosticar (cuánto ocupa un ciudadano, si un documento quedó certificado, si una firma se validó) sin tocar la confidencialidad del contenido. Los secretos (hashes, tokens, claves) no tienen ningún valor diagnóstico para un administrador -- solo lo tienen para alguien que quiera hacerse pasar por un ciudadano o por una entidad emisora, así que no hay ninguna razón operativa para mostrarlos ni siquiera truncados.

**Decisión (credencial en variable de entorno, sin modelo de usuarios).** No existe una tabla de administradores ni una pantalla de gestión de cuentas. Hay una sola credencial, cuyo hash Argon2id vive en la variable de entorno `ADMIN_PASSWORD_HASH`, verificada con la misma función que ya usa el ciudadano (`app.identidad.seguridad.verificar_password`). La sesión de la consola viaja en su propia cookie (`colcarpeta_admin_sesion`, con `path=/admin`), separada de la cookie del ciudadano, con las mismas protecciones (`HttpOnly`, `Secure`, `SameSite=Strict`) y su propio límite de intentos por origen, con el mismo patrón que ya usa el inicio de sesión del ciudadano (`app.identidad.servicios._intentos_fallidos`, derivado de `auditoria`, sin tabla propia).

**Justificación.** Un modelo completo de usuarios administradores (alta, baja, roles, recuperación de contraseña) es una funcionalidad entera por sí sola, y esta entrega no la pide: el caso de estudio habla de que el operador pueda observar su propio sistema, no de que administre un equipo de operadores con distintos niveles de acceso. Construir ese modelo ahora sería una funcionalidad no pedida que además necesitaría su propia superficie de pruebas y de seguridad (¿quién puede crear un administrador nuevo? ¿cómo se recupera una contraseña olvidada?) sin que nadie la haya solicitado todavía. Una credencial única en una variable de entorno resuelve exactamente el problema declarado -- que exista una única puerta de entrada, controlada por quien despliega la aplicación -- con el menor código posible, reutilizando toda la infraestructura de hash y de bloqueo por intentos que el proyecto ya tiene probada para el ciudadano. Sin el límite de intentos, la consola sería trivialmente un oráculo de fuerza bruta contra `ADMIN_PASSWORD_HASH`; con él, se comporta exactamente como el resto del sistema ya trata cualquier credencial.

**Consecuencias.** La consola vive bajo `/admin`, fuera del esquema OpenAPI. Si más adelante se necesita una acción real (reintentar una entrada de la bandeja de salida, por ejemplo), hace falta una decisión nueva y explícita, no una ampliación silenciosa de esta consola. La pantalla de transferencias no puede mostrar con certeza quién es el operador de origen de una transferencia entrante (CU-16): se resuelve por una heurística de host contra el directorio del MinTIC, la misma limitación de fondo que ya reconoce CLAUDE.md ("la API del centralizador no tiene autenticación de ningún tipo"). La pantalla de auditoría no muestra la columna `detalle` de ningún evento: revisar cada punto del código que escribe ahí para garantizar que ninguno registra jamás algo sensible resultaba más riesgoso que simplemente no exponerla nunca desde una consola de solo lectura.

**Matiz agregado el 2026-09-24: un enlace no es una puerta, pero solo en una dirección.** La consola enlaza al portal del ciudadano desde su propia cabecera -- una comodidad de navegación en el sentido en el que casi nunca hace falta, porque quien está en la consola llegó desde algún lado y probablemente quiera volver. El portal, en cambio, **no** enlaza a la consola en ningún lado, ni en su navegación ni en su pie: se descartó explícitamente un enlace discreto de pie de página que se había agregado y probado el mismo día, porque cualquier enlace hacia `/admin` visible en una pantalla que ve cualquier ciudadano -- por discreto que sea -- anuncia la existencia y la ruta exacta de la consola a un público mucho más amplio que quien la necesita, sin necesidad real: quien opera el sistema ya conoce la ruta. El enlace de la consola hacia el portal no relaja ninguna de las tres decisiones de arriba: la consola sigue exigiendo la contraseña única y su límite de intentos por origen exactamente igual que si el enlace no existiera.
